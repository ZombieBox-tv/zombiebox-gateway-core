...
export class Bridge {
  ...
  finish(success) {
    ...
    this.lastAck = pending.command.id;
  ...
  acknowledge(value) {
    if (
      ...
      !["PLAYING", "PAUSED", "STOPPED", "ENDED", "BUFFERING", "FAILED"].includes(value.state) ||
      !Number.isInteger(value.positionMs) ||
      value.positionMs < 0 ||
      value.positionMs > 604800000 ||
    ...
    if (
      value.commandId &&
      value.commandId !== this.pending?.command.id &&
      value.commandId !== this.lastAck
    )
      throw Error("stale_command");
    this.state = {
      state: value.state,
