...
async function run() {
  ...
  const yt = await Innertube.create({ lang: 'en', location: 'US', generate_session_locally: true,
    retrieve_player: operation === 'resolve', cookie, po_token: poToken, visitor_data: visitorData });
  if (operation === 'catalog') {
    ...
    for (const video of feed.videos ?? []) {
      ...
      seen.add(video.id);
      items.push({ id: video.id, title: String(video.title ?? '').slice(0, 500),
...
try { parentPort.postMessage({ ok: true, value: await run() }); }
