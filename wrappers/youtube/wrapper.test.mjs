...
test('interpreter evaluates without host access and stops infinite code', async () => {
  assert.deepEqual(await evaluate({ output: 'return {sig:"abc",n:"def"}' }), { sig: 'abc', n: 'def' });
  assert.equal(await evaluate({ output: 'return typeof process + ":" + typeof fetch + ":" + typeof require' }), 'undefined:undefined:undefined');
...
test('unauthorized calls do not start workers; public health stays local', async t => {
  let starts = 0;
  const { url } = await fixture(t, { workerFactory: () => { starts++; return new FakeWorker(); } });
...
test('concurrency, timeout and disconnect terminate workers', async t => {
  const workers = [];
  const { url } = await fixture(t, { timeoutMs: 100, workerFactory: () => { const w = new FakeWorker(); workers.push(w); return w; } });
...
test('failure response never exposes upstream secrets', async t => {
  const { url } = await fixture(t, { workerFactory: () => {
